...
export default function KdropDown(props: Props) {
  const [currentItem, setCrrentItem] = React.useState<KdropDownOption>(
    props.options[0]
  );

  const createHandleMenuClick = (menuItem: KdropDownOption) => {
    setCrrentItem(menuItem);
  ...
  return props.options.length > 1 ? (
    <Dropdown>
      <MenuButton>{currentItem.text} </MenuButton>
      <Menu slots={{ listbox: AnimatedListbox }}>
        {props.options &&
          props.options.map((option) => {
            return (
              <div>
                <MenuItem onClick={() => createHandleMenuClick(option)}>
                  {option.text}
                </MenuItem>
              </div>
            );
